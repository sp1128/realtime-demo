// OpenAI 流式对话。走 node:https 手写 SSE，而不是用 openai SDK 或 fetch。
//
// 原因就一个：代理。Node 内置的 fetch（undici）不认 https-proxy-agent，
// 而这个项目在国内跑，HTTPS_PROXY 是刚需。https.request 能直接吃 agent，
// 顺带也不用为了一个流式接口装一整个 SDK。
//
// 这一跳是整条链路里唯一走境外的，首 token 通常 300~600ms。
// 好在它的延迟能被 TTS 的播放时间盖掉一部分——第一句话一出来就开始播，
...
export class LlmStream {
  constructor(opts) {
    ...
    this.baseHost = opts.host || "api.openai.com";
