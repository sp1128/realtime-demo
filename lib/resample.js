...
export class Downsampler {
  ...
  process(int16) {
    ...
    // 三抽头低通太糊，5 抽头把 8kHz 附近压干净一点，辅音（s/c/q）不容易糊成别的字
    const lp = (i) =>
      (x[i - 2] + 4 * x[i - 1] + 6 * x[i] + 4 * x[i + 1] + x[i + 2]) / 16;

    const out = [];
    let p = this.pos;
    while (Math.floor(p) + 3 < n) {
    ...
    // 已经用不到的采样点扔掉，但要往前留两个做下一轮低通的左邻居
    const keepFrom = Math.max(0, Math.floor(p) - 2);
