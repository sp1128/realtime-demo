...
export class SentenceChunker {
  constructor(opts = {}) {
    ...
    // 英文的阈值比中文高得多，实测（只送一段、不 finishSession）：
    //   中文  5 字          → 339ms 出声
    //   英文  6 词/35 字符  → 3.5 秒不出声
    //         11 词/64 字符 → 1856ms
    //         26 词/145字符 → 358ms
    // 同一套数字套过去英文每轮开头都会卡两秒，所以按语种分开设
    this.en = opts.lang === "en";
    this.firstMin = opts.firstMin || (this.en ? 55 : 6);
    // 后续句子在软标点处切的最小长度。这时候首字延迟已经付过了，
    // 攒长一点让 TTS 拿到完整语义，语调更自然
    this.min = opts.min || (this.en ? 70 : 8);
    // 硬上限。有些模型会一路逗号写到底，不设上限就等于没切
    this.max = opts.max || (this.en ? 220 : 40);
    // 第一句的硬上限单独设，而且小得多：开场就来一句三十几个字不带标点的，
    // 按 max 等下去首字延迟会很难看
    this.firstMax = opts.firstMax || (this.en ? 160 : 16);
    this.buf = "";
