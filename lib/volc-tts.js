...
export class VolcTts extends EventEmitter {
  ...
  _sendStart(sid) {
    ...
    // additions 是个 JSON 字符串，直接打出来全是转义看不清，还原成对象再抛
    let additions = payload.req_params.additions;
    try { additions = JSON.parse(additions); } catch (e) {}
    this.emit("request", {
      type: "StartSession",
      sessionId: sid,
      resourceId: this.resourceId,
      params: { ...payload.req_params, additions },
    });
    this._sendEvent(EV_START_SESSION, sid, payload);
  }
  ...
  _drainPending() {
    ...
    this.busy = true;
    this.emit("request", { type: "槽位释放，补发排队的会话", sessionId: p.sid, params: {} });
    this._sendStart(p.sid);
    for (const t of p.texts) {
      this.emit("request", { type: "TaskRequest(补发)", sessionId: p.sid, params: { text: t } });
      this._sendEvent(EV_TASK_REQUEST, p.sid, { event: EV_TASK_REQUEST, req_params: { text: t } });
    }
    if (p.finish) {
      this.emit("request", { type: "FinishSession(补发)", sessionId: p.sid, params: {} });
      this._sendEvent(EV_FINISH_SESSION, p.sid, { event: EV_FINISH_SESSION });
    }
  }
  ...
  appendText(text) {
    ...
    this.emit("request", { type: "TaskRequest", sessionId: this.sessionId, params: { text } });
  ...
  finishSession() {
    ...
    if (this.pending && this.pending.sid === sid) {
      ...
      this.emit("request", { type: "FinishSession(排队中)", sessionId: sid, params: {} });
      return sid;
    }
    this.emit("request", { type: "FinishSession", sessionId: sid, params: {} });
