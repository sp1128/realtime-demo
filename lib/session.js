...
import { SYSTEM_PROMPT, interruptHint, pickBargeAck, toneHint, pickIdleNudge, pickIdleBye } from "./prompt.js";
...
export class CallSession extends EventEmitter {
  ...
  _onIdleTimeout() {
    ...
    if (this.idleNudges >= 1) {
      ...
      this.idleNudges = 1;
      const last = [...this.history].reverse().find((m) => m.role === "assistant");
      const line = pickIdleNudge(last && last.content, this.nudgeSeed++);
      this.emit("log", `⌛ 客户没接话，追一句「${line}」`);
      this._runTurn("", { say: line });
    }
  }
