...
import {
  buildSystemPrompt, interruptHint, pickBargeAck, toneHint,
  pickIdleNudge, pickIdleBye, guessCustomerName, NUDGE_TASK, sanitizeNudge,
  pickGreetStyle, pickFiller, nudgeTask,
} from "./prompt.js";
...
export class CallSession extends EventEmitter {
  constructor(cfg) {
    ...
    this.tts = new VolcTts({
      ...
      speaker: cfg.speaker,
      model: cfg.ttsModel,
      explicitLanguage: cfg.lang === "en" ? "en" : "zh-cn",
      styleInstruction: cfg.ttsStyle,
    ...
    // 语种贯穿三处：提示词、分句阈值、TTS 的 explicit_language
    this.lang = cfg.lang === "en" ? "en" : "zh";
    this.chunker = new SentenceChunker({ lang: this.lang });

    // 称呼要在建提示词之前定下来：它散在开场示例和确认身份的例句里
    this.customerName = (cfg.customerName || "").trim();
    this.history = [
      { role: "system", content: buildSystemPrompt(this.customerName, this.lang) },
    ];
    this.llm = null;
    ...
    this.nudgeSeed = 0;
    this.fillerSeed = 0;
    // LLM 首 token 要等 800ms 上下，这段时间里客户听到的是死寂。
    // 超过 fillerMs 还没吐字就先垫半个字，把等待盖在音频底下
  ...
  _wire() {
    ...
    this.asr.on("logid", (id) => this.emit("log", `🔖 ASR logid ${id}`));
    this.tts.on("request", (r) => this.emit("tts.request", r));
    this.tts.on("logid", (id) => this.emit("log", `🔖 TTS logid ${id}`));
  ...
  async _runTurn(userText, opts = {}) {
    ...
    // opts.say：跳过 LLM 直接念这句。追问用——那种话不需要生成，
    // 走 LLM 反而会让模型接着历史往下写、写成客户的台词
    const say = opts.say || "";
    const inject = !!opts.inject;
    if (say) {
      if (opts.hangupAfter) this.hangupAfter = true;
      this.chunker.reset();
      this.pendingText.length = 0;
      this.ttsFeedPaused = false;
      this.ttsFinishPending = false;
      ...
      this.playbackEndTimer = null;
      this.sentences = [];
      this.llmFullText = say;
      this.fedText = "";
      this.skipAckPrefix = "";
      this.ttsSession = this.tts.startSession();
      this._feedTts(say);
      this.emit("llm.delta", say);
      this.emit("llm.done", say);
      this.tts.finishSession();
      // 不用在这里写 history：_completeTurn 会拿 llmFullText 写进去，
      // 跟正常一轮走同一条收尾路径（包括等浏览器播完）
      return;
    }
    if (!inject) {
      this.hangupAfter = false;
    ...
    this.interruptedSpoken = "";
    const ack = interrupted ? pickBargeAck(userText, this.lang) : "";
    const tone = inject ? "" : toneHint(opts.meta, this.lang);
    if (opts.meta) {
      const m = opts.meta;
      this.emit(
        "log",
    ...
    // 反复参考一个已经过期的情绪
    const hints = [];
    if (interrupted) hints.push(interruptHint(ack, spoken, this.lang));
    if (tone) hints.push(tone);
    // 开场那一轮：随机换个措辞方向，别每通电话都是同一句
    if (!inject && userText === "（电话已接通）") {
      hints.push(pickGreetStyle(this.customerName, this.lang));
    }
    ...
    // 自适应垫话：只在 LLM 确实慢的时候才垫。
    // 开场那一轮不垫——一通陌生电话上来先"嗯"一声很怪。
    // 打断那一轮已经有 ack 了，不重复垫
    clearTimeout(this.fillerTimer);
    this.fillerTimer = null;
    const wantFiller =
      !inject && !ack && userText !== "（电话已接通）" && (this.cfg.fillerMs || 0) > 0;
    if (wantFiller) {
      this.fillerTimer = setTimeout(() => {
        this.fillerTimer = null;
        if (this.closed || this.ttsSession !== mySession) return;
        const f = pickFiller(userText, this.fillerSeed++, this.lang);
        this._feedTts(f);
        // LLM 待会儿多半也会以同一个音开头，去重，别出现"嗯，嗯，"
        this.skipAckPrefix = f.replace(/[。！？、，….\s]/g, "");
        this.emit("llm.delta", f);
      }, this.cfg.fillerMs);
    }
    ...
    try {
      ...
      // 用实际喂给 TTS 的文本当本轮内容，而不是 ack + LLM 原文：
      // 垫话和去重之后，两者是对不上的（会多出被 skipAckPrefix 去掉的那半个字）
      this.llmFullText = this.fedText || (ack || "") + full;
  ...
  // 记账"这一轮送进 TTS 的全文"。
  //
  // 英文句子之间要补一个空格，中文不用。不补的话会出现
  // "Mr. Smith?I'm calling" 这种——TTS 分开念听不出来，但文本是错的，
  // 而这段文本会作为 assistant 消息进历史，下一轮模型看到的就是粘连的句子；
  // 打断截断也是从它上面切的，切出来同样粘连
  _appendFed(text) {
    if (this.lang === "en" && this.fedText && !/\s$/.test(this.fedText)) {
      this.fedText += " ";
    }
    this.fedText += text;
  }
  ...
  _completeTurn() {
    ...
    this.skipAckPrefix = "";
    this.interruptedSpoken = "";
    // fedText 是真正送进 TTS 的文本，最能代表"客户听到了什么"
    const said = (
      this.fedText ||
      this.llmFullText ||
      this.sentences.map((s) => s.text).join("")
    ).trim();
  ...
  _onIdleTimeout() {
    ...
    // 已经在道别流程里了就别再来一次。道别那一轮结束时状态会回到 IDLE，
    // 这时如果还有一个早先挂上的计时器没清干净，就会把道别念第二遍
    if (this.hangupAfter) return;
    if (this.idleNudges >= 1) {
      const bye = pickIdleBye(this.nudgeSeed, this.lang);
      this.emit("log", `⌛ 客户还是没接话，收线「${bye}」`);
      this._runTurn("", { say: bye, hangupAfter: true });
    } else {
      this.idleNudges = 1;
      const last = [...this.history].reverse().find((m) => m.role === "assistant");
      // 客户到现在一个字都没说过？开场那句"（电话已接通）"是我们自己塞的，不算
      const firstContact = !this.history.some(
        (m) => m.role === "user" && !/^（.*）$/.test(m.content || ""),
      );
      const fallback = pickIdleNudge(last && last.content, this.nudgeSeed++, {
        firstContact,
        lang: this.lang,
        name: this.customerName || guessCustomerName(last && last.content),
      });
      // 开场一声没吭：那是在确认线路通不通，没上下文可结合，固定句就是最合适的。
      // 对话中途才让 LLM 结合上下文生成——固定句在具体语境里容易显得突兀
      if (firstContact) {
        this.emit("log", `⌛ 开场没人应，先确认线路「${fallback}」`);
        this._runTurn("", { say: fallback });
      } else {
        this._nudgeWithLlm(fallback, last && last.content);
      }
    }
  }
  ...
  // 结合上下文生成追问。跟正常一轮的两点不同：
  //   1. 指令用 **user 角色**发。历史最后一条是 assistant，再追加 system 的话
  //      模型会接着往下写对话的下一轮，也就是客户的台词（实测踩过）。
  //   2. **不流式**。先拿到全文校验合格再出声——追问就十几个字，等全文的代价
  //      很小，换来的是"模型跑偏时还能拦下来"。
  async _nudgeWithLlm(fallback, lastAssistant) {
    const llm = new LlmStream({
      apiKey: this.cfg.llmKey,
      model: this.cfg.llmModel,
      host: this.cfg.llmHost,
    ...
    let line = "";
    try {
      // 生成期间客户随时可能开口，超时就别等了，冷场已经够久
      const raw = await Promise.race([
        llm.run([...this.history, { role: "user", content: nudgeTask(this.lang) }], () => {}),
        new Promise((r) => setTimeout(() => r(""), this.cfg.nudgeTimeoutMs || 1500)),
      ]);
      line = sanitizeNudge(raw, lastAssistant, this.lang);
      if (raw && !line) {
        this.emit("log", `（追问不合格「${String(raw).trim().slice(0, 24)}」，退回固定句）`);
      }
    } catch (e) {
      this.emit("log", `（追问生成失败：${e.message}，退回固定句）`);
    } finally {
      llm.abort();
    }
    // 等的这段时间里客户开口了、或者通话结束了，就别再追问了
    if (this.closed || this.state !== "IDLE") return;
    const say = line || fallback;
    this.emit("log", `⌛ 客户没接话，追一句「${say}」${line ? "" : "（固定句）"}`);
    this._runTurn("", { say });
  }
