...
export class CallSession extends EventEmitter {
  constructor(cfg) {
    ...
    this.history = [
      { role: "system", content: buildSystemPrompt(this.customerName) },
    ];
    this.llm = null;
  ...
  async _runTurn(userText, opts = {}) {
    ...
    if (!inject && userText === "（电话已接通）") {
      hints.push(pickGreetStyle(this.customerName));
    }
    ...
    if (wantFiller) {
      this.fillerTimer = setTimeout(() => {
        ...
        const f = pickFiller(userText, this.fillerSeed++);
        this._feedTts(f);
  ...
  _onIdleTimeout() {
    ...
    if (this.idleNudges >= 1) {
      const bye = pickIdleBye(this.nudgeSeed);
  ...
  async _nudgeWithLlm(fallback, lastAssistant) {
    ...
    try {
      ...
      const raw = await Promise.race([
        llm.run([...this.history, { role: "user", content: NUDGE_TASK }], () => {}),
        new Promise((r) => setTimeout(() => r(""), this.cfg.nudgeTimeoutMs || 1500)),
      ]);
      line = sanitizeNudge(raw, lastAssistant);
      if (raw && !line) {
