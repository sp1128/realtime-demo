...
import { SYSTEM_PROMPT, interruptHint, pickBargeAck, IDLE_NUDGE_HINT, IDLE_BYE_HINT } from "./prompt.js";
...
export class CallSession extends EventEmitter {
    ...
    const messages = inject
      ...
      : interrupted
        ? [
            ...
            { role: "system", content: interruptHint(ack, spoken) },
  ...
  // 免提时 ASR 听到的常常是扬声器里正在念的词。跟当前这轮稿子一比就能看出来
  _looksLikeEcho(heard) {
    const spoken = (this.fedText || this.llmFullText || "").trim();
